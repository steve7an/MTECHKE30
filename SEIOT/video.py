...
class Video:
	...
	def compare(self):
		...
		for contour in contours:
			...
			if person.alerts[0] and person.alerts[1]:
				color = (0, 0, 255)
				cv2.line(self.frame, (x, y), (x + w, y + h), color, 2)
				cv2.line(self.frame, (x + w, y), (x , y + h), color, 2)
				detectStatus = "Alarm, fall detected and person is not moving"
					
				if not person.alarmReported:
					self.webservice.sound_alarm(self.settings.alarmSound)
					if not self.settings.windowMode:
						self.webservice.blink_alarm(self.settings.channel)

					file = "alarm_person{0}_time{1}.png".format(person.id,time.strftime("%Y%m%d_%H%M%S"))					
					cv2.imwrite(file, self.frame)

					if self.settings.triggerEmail:
						self.webservice.mail_alarm("not moving", person.id, file)
					
					self.storeAlarmImages(file)

					person.alarmReported = 1

			# Hide the rectangle drawn on the object
			if self.settings.debug:
				cv2.rectangle(self.frame, (x, y), (x + w, y + h), color, 2)
				cv2.putText(self.frame, "{}".format(cv2.contourArea(contour)), (x, y+h+20), 
					cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 1)
				#print ("contour area {0} and min area {1}".format(cv2.contourArea(contour), self.settings.minArea))
				cv2.putText(self.frame, "{} : {}".format(person.id, person.lastmoveTime), 
					(x, y+20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 1)
				print ("Person ID: {0} and last move time: {1}".format(person.id, person.lastmoveTime))
	...
	def turnGaussianOff(self):
		if self.settings.useGaussian:				
			self.settings.useGaussian = 0
			print("useGaussian: off")
			self.resetBackgroundFrame()

	def turnGaussianOn(self):
		if self.settings.useGaussian == 0:
